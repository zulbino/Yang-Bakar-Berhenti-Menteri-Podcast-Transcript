...
Senarai main sumber: https://www.youtube.com/playlist?list=PLqJKhYZQ8r9Uz3IPEh0lXpF17w3LQK62N

Hanya episod penuh (1+ jam) disertakan. Teaser pendek, klip sorotan, snippet kad-quote, dan episod format ringkas daripada senarai main yang sama tidak disertakan.
...
Episod yang ditranskrip melalui alternatif ASR tempatan (lihat [ARCHITECTURE.md](ARCHITECTURE.md#known-limitations), dalam Bahasa Inggeris) tiada pengesanan penutur (diarization) sebenar dalam `raw.md` -- nama penutur yang dipaparkan dalam `interview.md`/`interview-en.md`/`interview-ms.md` bagi episod ini adalah teka model tulisan semula berdasarkan konteks, bukan dibaca daripada label sumber. Anggap label tersebut belum disahkan, terutamanya dalam pertukaran cepat antara pelbagai penutur.
...
```bash
# Kemas kini semula indeks episod daripada senarai main (episod >= 1 jam sahaja)
