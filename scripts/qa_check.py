...
# --- table renderer, replaces the flat bullet list -------------------------------------
# The owner, 2026-09-14: "i want to change the qa checklist format a bit, hard to identify
# what model for each md files generated by one line of text, maybe we can have it in
# table, and with a checkmark and scores and blocker or notes column specific on whats
# pending (if any)"
#
# The old format put all four models in one sentence per episode (`models: raw.md=...,
# interview.md=...`), which is unreadable across 70 rows, and buried the pending work in
# prose. One row per episode now, one column per file, scores where they exist, and the
# pending column carries the SIGNATURES only. Full text stays in the detail section below,
# because an issue line runs to 300 characters and would destroy the table.
...
FILE_COLS = ["raw.md", "interview.md", "interview-en.md", "interview-ms.md"]


def short_model(value):
    """A model string that fits a table cell, without losing which engine it was."""
    if not value:
        return "--"
    v = str(value)
    for long, short in SHORT_MODEL.items():
        if v.startswith(long):
            return short + ("+" if len(v) > len(long) + 2 else "")
    return v.split("/")[-1][:14]


def gate_scores(slug, root):
    """(completeness, attribution) from the episode's last gate_rewrite try, or (--, --).

    These come from data/_<tag>_gate_rewrite.out, which is gitignored, so the numbers are
    derived locally and only exist for episodes the rewrite gate has run on. Absent is
    ...
    if not m:
        return "--", "--"
    log = root / "data" / f"_{m.group(1)}_gate_rewrite.out"
    if not log.exists():
        return "--", "--"
    tries = _re.findall(r"completeness (\d+)%.*?attribution ([\d.]+)%",
                        log.read_text(encoding="utf-8", errors="replace"))
    if not tries:
        return "--", "--"
    comp, attr = tries[-1]
    return f"{comp}%", f"{attr}%"


def ep_label(slug):
    """`ep03:bakar` where a bare tag is ambiguous, because both shows have ep01-ep06."""
    import re as _re
    m = _re.search(r"-(ep\d+)-", slug)
...
def main():
    ...
    lines = [
        ...
        "",
        "Newest first. One row per episode, one column per generated file, so the engine "
        "behind each file is readable without parsing a sentence.",
        "",
        "- **ok** -- `[x]` means no KNOWN failure signature fired. It does NOT mean the "
        "episode has been verified, and it does not mean raw.md came from the best "
        "engine: read the `raw.md` column for that.",
        "- **raw.md / interview / -en / -ms** -- the model that produced each file. `--` "
        "means no `model:` in frontmatter, which is every episode processed before the "
        "field existed. `MAI` is the best measured engine at 3.39% podcast WER; "
        "`whisper-med` is 20.52%.",
        "- **compl / attrib** -- completeness and attribution agreement with raw.md, from "
        "the episode's last `gate_rewrite.py` try. `--` where that gate has never run.",
        "- **pending** -- the signatures still open, detailed below. `(n waived)` counts "
        "issues reviewed as benign in `data/qa_reviewed.json`.",
        "",
        render_table(results, benign, ROOT),
        "",
    ]
    if broken:
        lines.append("## What is pending, per episode")
        lines.append("")
    ...
    if unprocessed:
        ...
        lines.append("")
    # The old "Clean episodes" list is gone: the table above carries every episode, clean
    # ones included, with their models in columns instead of a sentence. Listing them a
    # second time only made the file longer to scan.
    #
    # The benign section, on the other hand, was PROMISED by the header ("see the section
    # at the end") and never rendered -- a dangling cross-reference since the ledger was
    # introduced. Found in the 2026-09-14 doc-staleness audit. The rationale is the whole
    # point of data/qa_reviewed.json: without it a later session re-investigates an issue
    # somebody already settled, which is the amnesia the ledger exists to stop.
    if benign:
        lines.append("## Reviewed and judged benign")
        lines.append("")
