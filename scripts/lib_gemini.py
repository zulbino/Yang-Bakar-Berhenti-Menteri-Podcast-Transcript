...
2. Smooth out spoken awkwardness, filler words, false starts, and repetition, while preserving the speaker's true intent, tone, and key points. Do not invent or omit substantive claims. This is newspaper copy: drop filler words entirely ("uh", "um", "hmm", "aaa").
3. Preserve the language of each clause or phrase exactly as spoken, including mid-sentence code-switching (e.g. a sentence that starts in English and finishes in Malay must stay that way). Do not normalize a mixed-language sentence into a single language. Only polish grammar and remove filler within each language segment; do not translate any segment into the other language. You may add a short bracketed translation of an idiom only if it aids clarity. Malay must stay about as dense in your output as in the input: keep the Malay function words the speakers used ("yang", "tak", "ni", "tu", "dia", "kita", "sebab", "macam", "dengan", "untuk", "boleh", "kalau", "je", "pun") rather than rendering those clauses in English.
...
SUMMARY. One paragraph, in the language of the discussion, that names the episode's speakers and its main subjects. Use only what the transcript says.
